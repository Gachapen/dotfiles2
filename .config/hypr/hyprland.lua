-- Converted from hyprland.conf to the new Lua config format.
...
hl.on("hyprland.start", function()
	-- Import env into systemd/dbus and activate the session target so
	-- graphical-session.target comes up (required by xdg-desktop-portal,
	-- which screen sharing in Discord/etc. depends on).
	-- Target unit defined in: ~/.config/systemd/user/hyprland-session.target
	hl.exec_cmd("dbus-update-activation-environment --systemd --all")
	hl.exec_cmd("systemctl --user start hyprland-session.target")

	-- Required for gparted
	hl.exec_cmd("systemctl --user start hyprpolkitagent")

	-- GUI stuff
	hl.exec_cmd("hypridle & waybar & hyprpaper")

	-- Background apps
	hl.exec_cmd("nextcloud")

	-- Desktop apps
	hl.exec_cmd(terminal .. " & firefox & steam")
end)
...
hl.config({
	...
	animations = {
		-- The original config used `enabled = yes, please :)`, a hyprlang-only
		-- joke value; lua requires a strict boolean here.
		enabled = true,
	},
})
...
-- Memorite
hl.window_rule({ match = { title = "^Memorite.*" }, float = true })

-- Nier
hl.window_rule({ match = { title = "^NieR:Automata$" }, float = true })

-- Steam Big Picture
hl.window_rule({ match = { title = "^Steam Big Picture Mode$" }, fullscreen = true })

-- Elden Ring
hl.window_rule({ match = { title = "^ELDEN RING.*" }, float = true })
